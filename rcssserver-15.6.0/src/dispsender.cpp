...
DispSenderMonitorV3::sendShow()
{
    ...
    serializer().serializeBall( ostr,
                                stadium().ball() );
    // serializer().ballPositionOutput(
                                // stadium().ball() );

    serializer().SocketStadiumOutput( stadium().time(),
                             stadium().teamLeft(),
                             stadium().teamRight(),
                             stadium().ball() );
    ...
    for ( Stadium::PlayerCont::const_iterator p = stadium().players().begin();
          ...
          ++p )
    {   
        // serializer().playerPositionOutput( **p );
        ...
        serializer().SocketPlayerOutput(**p);
    }
...
DispSenderLoggerV4::sendShow()
{
    ...
    serializer().serializeBall( transport(),
                                stadium().ball() );
        // serializer().ballPositionOutput( transport(),
        //                         stadium().ball() );
    ...
    for ( Stadium::PlayerCont::const_iterator p = stadium().players().begin();
          ...
          ++p )
    {
        // serializer().playerPositionOutput( transport(), **p );
